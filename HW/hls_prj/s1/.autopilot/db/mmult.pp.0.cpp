...
#pragma HLS ARRAY_PARTITION variable = c_row_major dim = 1 factor = 8 cyclic
#pragma HLS ARRAY_PARTITION variable = c_row_major dim = 2 factor = 8 cyclic
...
#pragma HLS UNROLL factor = 8
 ap_int<8> a_forwarded = boundary_value;
        ...
        ap_int<8> b_forwarded = boundary_value;
        if (k < b_row && j < b_col) {
          b_forwarded = b_row_major[k][j];
        }

        ap_int<2 * 8> product = a_forwarded * b_forwarded;
...
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
 VITIS_LOOP_147_1: for (int j = 0; j < a_col; j++) {
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
#pragma HLS PIPELINE II = 1
...
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
 VITIS_LOOP_158_2: for (int j = 0; j < b_col; j++) {
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
#pragma HLS PIPELINE II = 1
...
writeC:
  for (int i = 0; i < a_row; i++) {
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
 VITIS_LOOP_187_5: for (int j = 0; j < c_col; j++) {
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
#pragma HLS PIPELINE II = 1
